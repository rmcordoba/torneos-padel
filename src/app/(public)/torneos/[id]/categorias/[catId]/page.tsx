...
export default async function PublicFixturePage({
  ...
  const [tc, session] = await Promise.all([
    getPublicCategoryFixture(catId, scopedOrg()),
    auth(),
  ]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      {/* Breadcrumb */}
      <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: "#475569", flexWrap: "wrap" }}>
        <Link href={plink("/torneos")} style={{ color: "#475569", textDecoration: "none" }}>Torneos</Link>
        <span>›</span>
        <Link href={plink(`/torneos/${tournamentId}`)} style={{ color: "#475569", textDecoration: "none", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 140 }}>
          {tc.tournament.name}
        </Link>
        <span>›</span>
        <span style={{ color: "#f1f5f9", fontWeight: 600 }}>{tc.category.name}</span>
      </div>

      {/* Header */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ width: 40, height: 40, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 12, background: ACCENT_BG, border: `1.5px solid ${ACCENT_BD}`, fontSize: 20 }}>
            🏆
          </div>
          <div>
            <h1 style={{ fontSize: 20, fontWeight: 700, color: "#f1f5f9", fontFamily: "var(--font-space), sans-serif" }}>{tc.category.name}</h1>
        ...
        {registrationOpen && (
          <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: "#64748b" }}>
            <span>👥</span>
            <span>
              <span style={{ fontWeight: 700, color: "#f1f5f9" }}>{approvedCount}</span>
              <span style={{ color: "#475569" }}>/{tc.maxTeams}</span>
      ...
      {/* Success banners */}
      {inscripto && (
        <div style={{ display: "flex", alignItems: "flex-start", gap: 12, borderRadius: 12, background: ACCENT_BG, border: `1px solid ${ACCENT_BD}`, padding: "14px 16px" }}>
          <span style={{ fontSize: 18, flexShrink: 0 }}>✅</span>
          <div>
            <p style={{ fontSize: 13, fontWeight: 700, color: ACCENT }}>Solicitud enviada</p>
      ...
      {espera && (
        <div style={{ display: "flex", alignItems: "flex-start", gap: 12, borderRadius: 12, background: "rgba(249,115,22,0.10)", border: "1px solid rgba(249,115,22,0.2)", padding: "14px 16px" }}>
          <span style={{ fontSize: 18, flexShrink: 0 }}>⏳</span>
          <div>
            <p style={{ fontSize: 13, fontWeight: 700, color: "#fb923c" }}>En lista de espera</p>
            <p style={{ fontSize: 12, color: "#94a3b8", marginTop: 2 }}>
          ...
          <div style={{ padding: 20 }}>
            {!session?.user ? (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12, padding: "16px 0", textAlign: "center" }}>
                ...
                <Link
                  href={`/login?callbackUrl=${encodeURIComponent(plink(`/torneos/${tournamentId}/categorias/${catId}`))}`}
                  style={{ display: "inline-flex", alignItems: "center", gap: 8, height: 36, padding: "0 20px", borderRadius: 12, background: ACCENT, color: "#0f172a", fontSize: 13, fontWeight: 800, textDecoration: "none" }}
                >
                  Iniciar sesión
                </Link>
              </div>
              ...
              <RegisterForm
                ...
                myFirstName={myProfile.firstName}
      ...
      {!hasFixture ? (
        <div style={{ borderRadius: 16, border: "2px dashed rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.02)", padding: "48px 0", textAlign: "center" }}>
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: "16px 20px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <h2 style={{ fontSize: 15, fontWeight: 700, color: "#f1f5f9", fontFamily: "var(--font-space), sans-serif" }}>{stage.name}</h2>
      </div>
      <div style={{ padding: 20, display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 20 }}>
        {stage.groups.map((group) => (
          <div key={group.id} style={{ borderRadius: 12, border: "1px solid rgba(255,255,255,0.07)", overflow: "hidden", background: "rgba(255,255,255,0.02)" }}>
            <div style={{ background: "rgba(255,255,255,0.04)", padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
              <h3 style={{ fontSize: 14, fontWeight: 700, color: "#f1f5f9", fontFamily: "var(--font-space), sans-serif" }}>{group.name}</h3>
            </div>
                  ...
                  <thead>
                    <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                      {["#", "Equipo", "PJ", "PG", "PP", "SG", "SP", "Pts"].map((h) => (
                        <th key={h} style={{ padding: "8px", textAlign: h === "Equipo" ? "left" : "center", fontSize: 11, fontWeight: 600, color: "#475569" }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {group.standings.map((s, idx) => {
                      ...
                      return (
                        <tr key={s.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)", background: idx < 2 ? "rgba(163,230,53,0.04)" : undefined }}>
                          <td style={{ padding: "10px 8px", fontWeight: 700, color: idx < 2 ? ACCENT : "#64748b" }}>{s.position}</td>
                          <td style={{ padding: "10px 8px", fontWeight: 600, color: idx < 2 ? ACCENT : "#94a3b8", maxWidth: 120, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{names.join(" / ")}</td>
                          ...
                          <td style={{ padding: "10px 8px", textAlign: "center", fontWeight: 700, color: idx < 2 ? ACCENT : "#64748b" }}>{s.points}</td>
                        </tr>
                      );
                    })}
                  </tbody>
            ...
            <div style={{ borderTop: "1px solid rgba(255,255,255,0.05)" }}>
              {group.matches.map((match, mi) => {
                ...
                return (
                  <div key={match.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", borderBottom: mi < group.matches.length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none" }}>
                    <span style={{ fontSize: 14, flexShrink: 0 }}>{done ? "✅" : "⏱"}</span>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                        <span style={{ fontSize: 12, fontWeight: 600, color: done && winner === t1?.teamId ? ACCENT : "#94a3b8" }}>
                          {t1?.team.players.map((p) => p.playerProfile.lastName).join("/")}
                        </span>
                        <span style={{ fontSize: 12, color: "#334155" }}>vs</span>
                        <span style={{ fontSize: 12, fontWeight: 600, color: done && winner === t2?.teamId ? ACCENT : "#94a3b8" }}>
                          {t2?.team.players.map((p) => p.playerProfile.lastName).join("/")}
                        </span>
                      </div>
                      {done && score && <p style={{ fontSize: 11, color: "#475569", marginTop: 2 }}>{score}</p>}
                    </div>
                  </div>
                );
              })}
            ...
            return (
              <div key={round} style={{ display: "flex", flexDirection: "column", width: 200 }}>
                <div style={{ textAlign: "center", marginBottom: 12 }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: "#475569", textTransform: "uppercase", letterSpacing: "0.05em" }}>{label}</span>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap }}>
                  {colNodes.map((node) => {
                    if (node.isBye && node.team) {
                      ...
                      return (
                        <div key={node.id} style={{ height: CARD_H, borderRadius: 12, border: `2px dashed ${ACCENT_BD}`, background: "rgba(163,230,53,0.05)", display: "flex", alignItems: "center", padding: "0 12px", gap: 8 }}>
                          <span style={{ fontSize: 12, fontWeight: 600, color: ACCENT, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1 }}>{names.join(" / ")}</span>
                          <span style={{ fontSize: 10, color: ACCENT, fontWeight: 700, flexShrink: 0 }}>BYE</span>
                        </div>
                      );
                    }
                    ...
                    if (!match) {
                      return (
                        <div key={node.id} style={{ height: CARD_H, borderRadius: 12, border: "2px dashed rgba(255,255,255,0.07)", background: "rgba(255,255,255,0.02)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                          <span style={{ fontSize: 12, color: "#334155" }}>Por definir</span>
                        </div>
                      );
                    }
                    const t1     = match.teams.find((t) => t.side === 1);
                    ...
                    const winner = match.result?.winnerId;
                    return (
                      <div key={node.id} style={{ height: CARD_H, borderRadius: 12, border: `1px solid ${done ? "rgba(163,230,53,0.2)" : "rgba(255,255,255,0.08)"}`, background: "rgba(12,22,45,0.8)", overflow: "hidden", display: "flex", flexDirection: "column" }}>
                        {[t1, t2].map((mt, i) => (
                          <div
                            key={i}
                            style={{ flex: 1, display: "flex", alignItems: "center", padding: "0 10px", gap: 6, background: done && winner === mt?.teamId ? "rgba(163,230,53,0.07)" : undefined, borderBottom: i === 0 ? "1px solid rgba(255,255,255,0.06)" : "none" }}
                          >
                            ...
                            <span style={{
                              ...
                              color: !mt ? "#334155" : done && winner === mt?.teamId ? ACCENT : "#94a3b8",
