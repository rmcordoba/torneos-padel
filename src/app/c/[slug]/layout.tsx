...
import { PortalHeader } from "@/app/(public)/_components/portal-header";
...
export default async function ClubLayout({
  ...
  const organizer = await resolveClubScope(slug);
  const basePath = `/c/${slug}`;

  const [session, liveMatches, showBookings] = await Promise.all([
    auth(),
    getPublicLiveMatches(organizer.id),
    hasBookings(organizer.id),
  ]);
  ...
  return (
    <div className="portal-bg">
      <PortalHeader
        ...
        basePath={basePath}
        brand={{ name: organizer.name, logoUrl: organizer.logoUrl }}
        showBookings={showBookings}
      />

      {/* Live ticker (scopeado al club) */}
        ...
        <div style={{ maxWidth: MAX, margin: "0 auto", padding: "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{
              ...
              {organizer.logoUrl
                ? <img src={organizer.logoUrl} alt={organizer.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                : "🎾"}
            </div>
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#f1f5f9", fontFamily: "var(--font-space), sans-serif" }}>{organizer.name}</div>
