...
/* ── design tokens ── */
const A   = "#a3e635";
const C   = { bg: "oklch(16% 0.012 250)", border: "oklch(26% 0.01 250)" };
const HDR = "oklch(19% 0.012 250)";
const ROW = "oklch(22% 0.01 250)";
...
function DataRow({
  ...
  return (
    <div
      ...
      style={{
        ...style,
        background: hover ? "oklch(21% 0.012 250)" : "transparent",
...
function StatCard({
  ...
  return (
    <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 13, padding: "16px 18px", display: "flex", alignItems: "center", gap: 14 }}>
      ...
      <div>
        <div style={{ fontFamily: "Space Grotesk, sans-serif", fontSize: 26, fontWeight: 800, color, lineHeight: 1 }}>{value}</div>
...
function StatusNum({ value, color }: { value: number; color: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}>
      ...
      <span style={{ fontSize: 14, fontWeight: 800, color, fontFamily: "Space Grotesk, sans-serif" }}>{value}</span>
    </div>
  );
}
...
function ProgressBar({ value, total }: { value: number; total: number }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      <div style={{ flex: 1, height: 5, background: "oklch(25% 0.01 250)", borderRadius: 3, overflow: "hidden" }}>
        <div style={{ width: `${pct}%`, height: "100%", background: clr, borderRadius: 3, transition: "width .3s" }} />
      </div>
      <span style={{ fontSize: 11, fontWeight: 700, color: clr, minWidth: 34, textAlign: "right", fontFamily: "Space Grotesk, sans-serif" }}>{pct}%</span>
    </div>
  );
}

function DownloadBtn({ type }: { type: string }) {
  return (
    <a
...
function ColHeader({ cols, templateCols }: { cols: React.ReactNode[]; templateCols: string }) {
  return (
    <div style={{ display: "grid", gridTemplateColumns: templateCols, alignItems: "center", padding: "11px 20px", background: HDR, borderBottom: `1px solid ${C.border}` }}>
      {cols.map((h, i) => (
        <span key={i} style={{ fontSize: 10, fontWeight: 700, color: T.xlo, textTransform: "uppercase", letterSpacing: "0.08em" }}>
          {h}
        </span>
      ))}
    </div>
...
function TournamentGroupRow({ name }: { name: string }) {
  return (
    <div style={{ padding: "8px 20px", background: "oklch(20% 0.012 250)", borderBottom: `1px solid ${ROW}`, display: "flex", alignItems: "center", gap: 8 }}>
...
function SectionHeader({ title, count, downloadType }: { title: string; count: number; downloadType: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px", background: HDR, borderBottom: `1px solid ${C.border}` }}>
      <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
        <span style={{ fontSize: 14, fontWeight: 700, color: T.hi, fontFamily: "Space Grotesk, sans-serif" }}>{title}</span>
...
function TabInscriptos({ rows }: { rows: RegistrationRow[] }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {tkeys.length === 0 ? (
        ...
        <TableContainer>
          <SectionHeader title="Inscriptos por categoría" count={rows.length} downloadType="inscripciones" />
          <ColHeader
            templateCols={COLS}
              ...
              <Fragment key={tid}>
                ...
                {t.rows.map((row, i) => {
                  ...
                  return (
                    ...
                    >
                      <span style={{ fontSize: 13, fontWeight: 600, color: T.hi, paddingLeft: 14 }}>
                        {row.categoryName}
                      </span>
                      <span style={{ fontSize: 15, fontWeight: 800, color: T.hi, textAlign: "center", fontFamily: "Space Grotesk, sans-serif" }}>
                        {row.total}
                      </span>
          ...
          <div style={{ display: "grid", gridTemplateColumns: COLS, alignItems: "center", padding: "12px 20px", borderTop: `1px solid ${C.border}`, background: HDR }}>
            ...
            <span style={{ fontSize: 15, fontWeight: 800, color: T.hi, textAlign: "center", fontFamily: "Space Grotesk, sans-serif" }}>{totalSum}</span>
...
function TabPartidos({ rows }: { rows: MatchRow[] }) {
  const totalSum  = rows.reduce((s, r) => s + r.total, 0);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {tkeys.length === 0 ? (
        ...
        <TableContainer>
          <SectionHeader title="Partidos por etapa" count={rows.length} downloadType="partidos" />
          <ColHeader
            templateCols={COLS}
          ...
          {tkeys.map((tid, tIdx) => {
            ...
            return (
              <Fragment key={tid}>
                ...
                {t.rows.map((row, i) => {
                  const isLast = i === t.rows.length - 1 && tIdx === tkeys.length - 1;
                  const pct = row.total > 0 ? Math.round((row.played / row.total) * 100) : 0;
                  return (
                    <DataRow
...
function TabCampeones({ rows }: { rows: ChampionRow[] }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      {rows.length === 0 ? (
        ...
        <TableContainer>
          <SectionHeader title="Campeones por torneo" count={rows.length} downloadType="campeones" />
          <ColHeader
            templateCols={COLS}
          ...
          {rows.map((r, i) => (
            ...
            >
              ...
              <span style={{ fontSize: 11, padding: "3px 8px", borderRadius: 12, background: "rgba(163,230,53,.08)", color: T.md, border: "1px solid oklch(28% 0.01 250)", whiteSpace: "nowrap" }}>
  ...
  return (
    <div style={{ maxWidth: 1020 }}>
      ...
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700, fontFamily: "Space Grotesk, sans-serif", color: T.hi }}>
          Reportes
        </h1>
          ...
          return (
            ...
            >
              ...
              <span style={{
                ...
                background: active ? "rgba(163,230,53,.15)" : "oklch(22% 0.01 250)",
      ...
      {/* Content */}
      {tab === "inscriptos" && <TabInscriptos rows={registrations} />}
      {tab === "partidos"   && <TabPartidos   rows={matches} />}
      {tab === "campeones"  && <TabCampeones  rows={champions} />}
    </div>
  );
}
