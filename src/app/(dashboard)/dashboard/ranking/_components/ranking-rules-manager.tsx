  ...
  return (
    <div style={{ background: "var(--bg-surface)", borderRadius: 12, border: "1px solid var(--border-default)", overflow: "hidden" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px", borderBottom: "1px solid var(--border-subtle)" }}>
        <h3 style={{ fontSize: 14, fontWeight: 700, color: "var(--text-primary)", fontFamily: "Space Grotesk, sans-serif" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {rules.map((rule) => (
              <div key={rule.id} style={{
                ...
                <span style={{ fontSize: 14, fontWeight: 800, color: "var(--accent)", fontFamily: "Space Grotesk, sans-serif", flexShrink: 0 }}>
                  {rule.points} pts
                </span>
                <button
                  type="button"
                  ...
                  style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-darkest)", opacity: (deletingId === rule.id || addPending) ? 0.5 : 1 }}
                >
                  {deletingId === rule.id
