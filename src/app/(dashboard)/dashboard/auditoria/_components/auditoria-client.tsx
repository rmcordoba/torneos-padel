...
function exportCSV(entries: AuditEntry[]) {
  const headers = ["Fecha","Hora","Usuario","Email","Entidad","Acción","Detalle","Torneo","IP"];
...
export function AuditoriaClient({ entries }: { entries: AuditEntry[] }) {
  ...
  const activeUsers = useMemo(
    () => new Set(entries.filter((e) => e.createdAt.slice(0, 10) === todayKey).map((e) => e.userEmail)).size,
    [entries, todayKey]
  );
  ...
  const filtered = useMemo(() => {
    ...
    return entries.filter((e) => {
      if (q && !e.userName.toLowerCase().includes(q) && !e.userEmail.toLowerCase().includes(q) &&
          !buildDetail(e).toLowerCase().includes(q) && !(ENTITY_LABEL[e.entity] ?? e.entity).toLowerCase().includes(q)) return false;
      if (filterGroup  !== "todas"  && getGroup(e.entity) !== filterGroup)  return false;
      if (filterAction !== "todas"  && e.action             !== filterAction) return false;
      return true;
    });
  }, [entries, search, filterGroup, filterAction]);
          ...
          <div style={{ background: "rgba(12,20,40,0.7)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 13, padding: "16px 18px" }}>
            ...
            {(() => {
              ...
              for (const e of todayEntries) {
                ...
                if (!existing || e.createdAt > existing.lastAt) {
