...
export class RoundRobinEngine implements FormatEngine {
  ...
  async recalculateGroupStandings(groupId: string): Promise<void> {
    ...
    for (const match of matches) {
      ...
      if (match.result?.winnerId === t1.teamId) {
        stats[t1.teamId].won++; stats[t1.teamId].pts += 2; stats[t2.teamId].lost++;
      } else if (match.result?.winnerId === t2.teamId) {
        stats[t2.teamId].won++; stats[t2.teamId].pts += 2; stats[t1.teamId].lost++;
      }
    }
